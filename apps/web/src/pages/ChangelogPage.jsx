...
const RELEASES = [
  {
    version: 'v1.3', date: 'May 2026', latest: true,
    summary: 'Insurance Analyzer launch, Maintenance Scheduler improvements, and document upload fixes.',
    new: ['Insurance Analyzer — coverage gap detection, renewal alerts, and policy comparison', 'CompassHomeOS Preview page — white-label enterprise preview', 'Agent Value and Brokerage Value marketing pages', 'Tier-4 info pages: Pricing, Security, About, Contact'],
    improved: ['Maintenance Scheduler — new Smart Recommendations tab with seasonal and predictive alerts', 'Valuation & Equity Dashboard — 5-year forecast with three scenario projections', 'Portfolio Overview — Risk Indicators section with auto-detected risks across properties', 'Home Profile — unified Quick Actions bar linking all modules'],
    fixed: ['Document upload progress indicator stalling on large files', 'Utility billing cycle day calculation edge case on month-end dates', 'Insurance renewal countdown showing negative days after expiry'],
  },
  {
    version: 'v1.2', date: 'March 2026', latest: false,
    summary: 'Portfolio Overview, Valuation & Equity Dashboard, and Maintenance Scheduler released.',
    new: ['Portfolio Overview — multi-property intelligence with financial summary and risk scoring', 'Valuation & Equity Dashboard — 12-month trend chart, equity breakdown, and renovation ROI', 'Maintenance Scheduler — calendar view, task list, cost forecast, and smart recommendations', 'HomeOS Overview and Why HomeOS Exists marketing pages'],
    improved: ['Home Profile — all 9 sections per spec with valuation snapshot and timeline preview', 'Bill Pay — Next Bill Due widget and category breakdown', 'Home Valuation — comparable sales tab and renovation scenario planner'],
    fixed: ['Property tax page breadcrumb navigation returning 404', 'Timeline event date sorting on leap year edge case', 'Dashboard alert count not updating on mark-complete action'],
  },
  {
    version: 'v1.1', date: 'January 2026', latest: false,
    summary: 'Module polish pass — standardized headers, smart insights, design tokens applied site-wide.',
    new: ['Smart Insights bars across Utilities, Insurance, and Maintenance pages', 'Efficiency Score in Utilities Dashboard', 'Annual Forecast metric in Utilities and Bill Pay', 'Export Summary buttons across all financial modules'],
    improved: ['Standardized page headers with breadcrumb navigation on all module pages', 'Design token consistency pass (12px radius, spacing, shadow-sm) across all pages', 'Bill Pay — standardized to match Tier-2 design system', 'Documents Vault — missing documents section with smart alerts'],
    fixed: ['Warranty Tracker expiration alerts not surfacing in Dashboard', 'Plants/Landscaping seasonal guide not rendering on mobile', 'Rental Properties list view pagination resetting on filter change'],
  },
  {
    version: 'v1.0', date: 'November 2025', latest: false,
    summary: 'Initial HomeOS platform launch with core modules and PocketBase backend.',
    new: ['Dashboard — unified home command center', 'Maintenance Management — task tracking and vendor coordination', 'Bill Pay — bills, expenses, and payment links', 'Home Valuation — AVM integration and comparable sales', 'Documents — encrypted vault with category organization', 'Insurance Analyzer — policy tracking and coverage analysis', 'Utilities — provider management and budget tracking', 'Warranty Tracker — warranty storage and expiration alerts', 'Home Timeline — property event history', 'Learning Hub — homeowner education and guides', 'Property Tax — assessment tracking and appeal guidance', 'Rental Properties — multi-unit management', 'Plants/Landscaping — service schedule and seasonal guide', 'Ready to Sell — pre-listing checklist'],
    improved: [], fixed: [],
  },
];
...
const ReleaseEntry = ({ release, defaultOpen = false }) => {
  ...
  return (
    <div style={{ background: 'white', borderRadius: '16px', border: `1px solid ${release.latest ? '#c7d7eb' : '#e2e8f0'}`, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
      {release.latest && <div style={{ height: '3px', background: 'linear-gradient(90deg, #1e3a5f, #e8604c)' }} />}
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between text-left" style={{ padding: '20px 28px', background: 'none', border: 'none', cursor: 'pointer' }}>
        <div className="flex items-center gap-4">
          <div>
            <div className="flex items-center gap-2" style={{ marginBottom: '4px' }}>
              ...
              {release.latest && <span style={{ fontSize: '11px', fontWeight: 700, color: '#1e3a5f', background: '#eef2f8', padding: '2px 8px', borderRadius: '999px' }}>Latest</span>}
            </div>
        ...
        <div className="flex items-center gap-3 flex-shrink-0 ml-4">
          ...
          {open ? <ChevronUp style={{ width: '18px', height: '18px', color: '#94a3b8' }} /> : <ChevronDown style={{ width: '18px', height: '18px', color: '#94a3b8' }} />}
        </div>
      </button>
      {open && (
...
const ChangelogPage = () => (
  <SiteLayout seo={{ title: 'Changelog — HomeOS' }} fullWidth>

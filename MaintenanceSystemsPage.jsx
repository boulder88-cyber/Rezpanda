...
import { Plus, Edit, Trash2, Eye, Bed, Bath, Maximize } from 'lucide-react';
...
const PropertiesList = () => {
  ...
  const loadProperties = async () => {
    try {
      const records = await pb.collection('properties').getFullList({ sort: '-created', $autoCancel: false });
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Manage your property portfolio" />
      </Helmet>
                  ...
                  <Card key={property.id} className="overflow-hidden">
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-3">
                        <h3 className="font-semibold text-slate-900 text-lg">{property.address}</h3>
                        <span className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor(property.status)}`}>
                          {property.status}
                        </span>
                      </div>

                      <p className="text-sm text-slate-600 mb-3 capitalize">{property.propertyType}</p>
